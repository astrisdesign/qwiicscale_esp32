...
class NAU7802():
    # Default constructor
    def __init__(self, deviceAddress = 0x2a, zeroOffset = False,#i2c, deviceAddress = 0x2A, zeroOffset = False,
                 calibrationFactor = False):
        self.bus = board.I2C()    # This stores the user's requested i2c bus
        self.deviceAddress = deviceAddress    # Default unshifted 7-bit address of the NAU7802

        # y = mx + b
        self.zeroOffset = zeroOffset;    # This is b in y = mx + b.
    ...
    def calculateZeroOffset(self, averageAmount=10):    # Also called taring. Call this with nothing on the scale
        self.ZeroOffset = self.getAverage(averageAmount)
    ...
    # Implemented to get around the smbus dependency in the original library.
    def read_byte(self, address):
        buffer = bytearray(1)
        while not self.bus.try_lock():
            pass
        self.bus.readfrom_into(address, buffer)
        self.bus.unlock()
        return buffer[0]

    # Implemented to get around the smbus dependency in the original library.
    def write_byte(self, registerAddress, value):
        while not self.bus.try_lock():
            pass
        self.bus.writeto(self.deviceAddress, bytes([registerAddress]) + bytes([value]))
        self.bus.unlock()
    ...
    def isConnected(self):    # Returns true if device acks at the I2C address
        try:
            self.read_byte(self.deviceAddress)
            return True
        except:
            return False
    ...
    def calculateCalibrationFactor(self, weightOnScale, averageAmount):    # Call this with the value of the thing on the scale. Sets the calibration factor based on the weight on scale and zero offset.
        reading = self.getAverage(averageAmount)
        newCalFactor = (reading - self.zeroOffset) / weightOnScale
    ...
    def getAverage(self, averageAmount, timeout=1):    # Return the average of a given number of readings
        total = 0
        samplesAcquired = 0
        ...
        while True:
            ...
            if time.time() - startTime > timeout:
                print(f'Exceeded timeout limit of {timeout}s.')
    ...
    def getReading(self):    # Returns 24-bit reading. Assumes CR Cycle Ready bit (ADC conversion complete) has been checked by .available()
        ...
        block = self.read_i2c_block_data(self.deviceAddress, Scale_Registers['NAU7802_ADCO_B2'], 3)
    ...
    # Implemented to get around the smbus dependency in the original library.
    def read_i2c_block_data(self, deviceAddress, registerAddress, length):
        try:
            buffer = bytearray(length)
            while not self.bus.try_lock():
                pass
            self.bus.writeto(deviceAddress, bytes([registerAddress]))
            time.sleep(0.001)
            self.bus.readfrom_into(deviceAddress, buffer)
            # time.sleep(0.001)
            self.bus.unlock()
            return list(buffer)
    ...
    def getRegister(self, registerAddress):    # Get contents of a register
        try:
            return self.read_i2c_block_data(self.deviceAddress, registerAddress, 1)[0]
        except Exception as e:
            print("Error reading register: " + str(e))
            return False    # Error
    ...
    def setRegister(self, registerAddress, value):    # Send a given value to be written to given address. Return true if successful
        try:
            ...
            self.write_byte(registerAddress, value)
        except:
